...
//	this->addParticle();
	...
	schedule(schedule_selector(GameScene::shootBullet), 0.5);  //发射子弹
//	schedule(schedule_selector(GameScene::update), 1.f);
	//更新子弹
//	schedule(schedule_selector(GameScene::shootPlane));   //发射敌机
...
void GameScene::shootBullet() {
	...
	//
	addChild(cat, PLANE_ORDER, TAG_Bullet);  //TODO Bullet Order ，添加到图层
